...
diary(['./log/toys_' datestr(datetime('now'),'yy-mm-dd-HH-MM-SS') '.txt'])
for sparse_ratio = 0.05: 0.05: 0.9
    X = L;
    ind = find(rand(n1*n2*n3,1)<sparse_ratio);
    ...
    disp(['sparse_ratio=' num2str(sparse_ratio), ', iter ' num2str(iter) ', mu=' num2str(mu) ', obj=' num2str(obj) ', err=' num2str(err) ', psnr=' num2str(psnr)]);
    obj = tnnL+lambda*norm(Sparse(:),1);
    err = norm(dY(:));
end
diary off